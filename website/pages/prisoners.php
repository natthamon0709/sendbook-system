          ...
          <tr>
            <td class="">
              <div class="form-check checkbox-lg">
                <input class="form-check-input primary hover" type="checkbox" value="" name="check_prisoner[<?php echo $row["id"]; ?>]" id="check_prisoner[<?php echo $row["id"]; ?>]">
              </div>
            </td>
            <td>
              <?php echo $row["id"] ?>
            ...
            <td>
              <?php echo getPrisonerStatusBadge($row); ?>
            </td>
            <td width="20%">
...
<script>
  ...
  async function handleOnPrintClick() {
    ...
    window.open("./print-prisoners.php?" + args);
...
<script>
  ...
  function handleUploadFile(file) {
    if (file) {
      if (file.size > 0) {
        ...
        if (['xlsx', 'xlsm', 'xltx', 'xltm'].includes(fileExtension)) {
          uploadFile(file);
        }
      }
    }
  ...
  async function uploadFile(file) {
    ...
    try {
      const response = await fetch('upload-prisoners.php', {
        method: 'POST',
        body: formData
      })
